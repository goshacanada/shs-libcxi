...
/* CXI RDMA write bandwidth benchmark */
...
int write_bw_alloc_ini(struct util_context *util)
{
	...
	/* Config */
	ini_opts.alloc_hrp = (opts->use_hrp && opts->min_size <= PORTALS_MTU);
	...
	if (opts->use_hrp && opts->min_size <= PORTALS_MTU) {
	...
	if (!opts->use_idc || opts->max_size > MAX_IDC_RESTRICTED_SIZE ||
	    (opts->unrestricted &&
	     opts->max_size > MAX_IDC_UNRESTRICTED_SIZE)) {
	...
	if (opts->use_idc && (opts->min_size <= MAX_IDC_UNRESTRICTED_SIZE ||
			      (!opts->unrestricted &&
			       opts->min_size <= MAX_IDC_RESTRICTED_SIZE))) {
		/* C State */
		...
		/* IDC Put */
		util->idc_cmd.idc_put.idc_header.command.opcode =
			C_CMD_NOMATCH_PUT;
...
int do_single_iteration(struct util_context *util)
{
	...
	for (i = 0; i < opts->list_size; i++) {
		if (!opts->use_idc ||
		    opts->max_size > MAX_IDC_RESTRICTED_SIZE ||
		    (opts->unrestricted &&
		     opts->max_size > MAX_IDC_UNRESTRICTED_SIZE)) {
			...
			rc = cxi_cq_emit_nomatch_dma(
				cxi->ini_cq, &util->dma_cmd.nomatch_dma);
			if (rc) {
				fprintf(stderr,
...
void usage(void)
{
	...
	printf("  cxi_write_bw ADDR [OPTIONS]\n");
	...
	printf("                         If (size * list_size) > buf_sz, writes will overlap\n");
	printf("                         (default: %lu)\n", MAX_BUF_SIZE_DFLT);
	printf("      --buf-align=ALIGN  Byte-alignment of writes in the buffer (default: %u)\n",
	       BUF_ALIGN_DFLT);
...
int main(int argc, char **argv)
{
	...
	if (opts->max_size > opts->max_buf_size)
		errx(1, "Max RDMA buffer size (%lu) < max write size (%lu)!",
	...
	if (opts->use_hrp && opts->unrestricted) {
		opts->use_hrp = 0;
	...
	snprintf(util.header, MAX_HDR_LEN, "%*s  %*s  %*s  %*s", SIZE_W,
		 "RDMA Size[B]", COUNT_W, "Writes", BW_W, "BW[MB/s]", MRATE_W,
		 "PktRate[Mpkt/s]");
	print_separator(strlen(util.header));
	printf("    CXI RDMA Write Bandwidth Test\n");
	...
	if (opts->min_size == opts->max_size) {
		...
		printf("Min Write Size   : %lu\n", opts->min_size);
		printf("Max Write Size   : %lu\n", opts->max_size);
	}
	printf("List Size        : %u\n", opts->list_size);
	printf("HRP              : %s\n", hrp_opt_override ?
	...
	if (using_tx) {
		rc = write_bw_alloc_ini(&util);
		if (rc < 0)
			goto cleanup;
	}
	if (using_rx) {
		rc = write_bw_alloc_tgt(&util);
		if (rc < 0)
			goto cleanup;
	}
	...
	for (util.size = opts->min_size; util.size <= opts->max_size;
		...
		/* Change to non-HRP communication profile above 2048B */
		if (opts->use_hrp &&
		    (opts->max_size > PORTALS_MTU &&
		     util.size == PORTALS_MTU) &&
		    using_tx) {
			rc = set_to_dflt_cp(&util, cxi->ini_cq);
			if (rc)
				break;
		}
	}
